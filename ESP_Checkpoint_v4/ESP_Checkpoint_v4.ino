// ============================================================
// ESP_Checkpoint_v4.ino — TUG Test: Checkpoint & RGB Controller
// ============================================================
// บอร์ดนี้ติดตั้งที่จุดหมุนตัวระยะ 3 เมตร ใช้ได้ 3 แบบ (เลือกแหล่งจับเวลาที่เว็บ "ตั้งค่าอุปกรณ์"):
//   ① ฮาร์ดแวร์: เก้าอี้ + checkpoint — รอ START จากเก้าอี้ แล้วส่ง CHECKPOINT กลับ (เหมือน v2/v3)
//   ② กล้อง + checkpoint (มีหรือไม่มีเก้าอี้ก็ได้) — "เฝ้าตลอด" เห็นคนเดินผ่านเมื่อไรก็จดเวลาจริง
//      (NTP, ms) แล้วส่งขึ้น device_status/checkpoint (pass_at_ms) ให้เว็บคิดขาไป/ขากลับกับเวลาของกล้อง
//   ③ กล้องอย่างเดียว — ไม่ต้องเปิดบอร์ดนี้
// เซนเซอร์อ่านใน task แยก (sensorTask) การคุย Firestore ในลูปจึงไม่ทำให้พลาดจังหวะคนเดินผ่านอีก
//
...
// ---------- โหมดเฝ้าตลอด (จับเวลาจากกล้อง / ไม่มีเก้าอี้) ----------
// เห็นคนเดินผ่านแล้ว ต้องพ้นระยะต่อเนื่องเท่านี้ และห่างจากครั้งก่อนอย่างน้อยเท่านี้ จึงนับครั้งใหม่
// (กันนับซ้ำตอนผู้ทดสอบหมุนตัวอยู่หน้าเซนเซอร์)
#define FREE_RUN_CLEAR_MS        1000
#define FREE_RUN_REFRACTORY_MS   4000
#define PASS_LIGHT_MS            3000    // ไฟขาวค้างหลังเห็นคนเดินผ่าน (ตอนไม่มีเก้าอี้บอกขั้นตอน)
#define SETTINGS_POLL_INTERVAL_MS 12000  // อ่านแหล่งจับเวลาจาก device_commands/settings
// ไม่ได้ยินเก้าอี้นานเท่านี้ = เก้าอี้ไม่อยู่ → ไม่เอาสถานะเก้าอี้ค้างเก่ามาขับไฟ และเฝ้าตลอดเอง
#define CHAIR_ABSENT_MS          15000
// โหมดกล้อง: ไฟตามขั้นตอนของหน้ากล้อง (device_status/camera) — อ่านถี่ตอนใกล้/กำลังทดสอบ ห่างตอนว่าง
// (ประหยัดโควตาอ่าน Firestore: เปิดทิ้งไว้ว่าง ๆ ทั้งวัน ≈ 10,800 ครั้ง/วัน)
#define CAM_POLL_ACTIVE_MS       1000    // นั่งพร้อม / กำลังเดิน — ไฟเขียว→ขาวต้องเปลี่ยนเร็ว
#define CAM_POLL_NEAR_MS         2000    // รอผู้ทดสอบนั่ง / พักหลังจบรอบ
#define CAM_POLL_IDLE_MS         8000    // หน้ากล้องปิด/ซ่อน/ยังไม่ได้ภาพ
#define CAM_STALE_SEC            25      // หน้ากล้องไม่ส่งสถานะนานกว่านี้ = ปิดไปแล้ว (ปกติส่งทุก 10 วิ)
#define CAM_STALE_HIDDEN_SEC     90      // หน้ากล้องที่ถูกซ่อน เบราว์เซอร์ให้ส่งได้แค่นาทีละครั้ง
...
//
// magic บอกประเภทค่า:
//   "TUGWIFI" = ค่าที่เก้าอี้ต่อติดแล้ว → จำไว้เลย
//   "TUGWTRY" = ค่าที่ผู้ใช้เพิ่งกดเชื่อมต่อที่เก้าอี้ ยังไม่รู้ว่ารหัสถูกไหม
//               → ลองต่อตาม จำเฉพาะเมื่อติด ไม่ติดก็กลับไปวงเดิมทันที
#define TUG_WIFI_MAGIC     "TUGWIFI"
#define TUG_WIFI_MAGIC_TRY "TUGWTRY"
...
// ---------- โหมดเฝ้าตลอด + เหตุการณ์ "คนเดินผ่าน" (sensorTask ↔ loop) ----------
// sensorTask เขียน passMillis ก่อนแล้วค่อยเพิ่ม passCount — ลูปเห็น passCount เปลี่ยนจึงอ่าน passMillis
volatile bool          timingCamera = false;   // เว็บตั้ง "จับเวลาจากกล้อง" (อ่านจาก device_commands/settings)
volatile bool          freeRun      = false;   // กำลังเฝ้าตลอด (ลูปคำนวณ sensorTask อ่าน)
volatile uint32_t      passCount    = 0;       // จำนวนครั้งที่เห็นคนเดินผ่านตั้งแต่บูต
volatile unsigned long passMillis   = 0;       // millis() ของจังหวะที่เริ่มเห็นคนในระยะ (ครั้งล่าสุด)
volatile bool          hwPassFlag   = false;   // เห็นคนผ่านระหว่าง CP_DETECTING — ลูปส่ง CHECKPOINT ให้เก้าอี้
volatile bool          detectReset  = true;    // ลูปสั่งล้างสถานะตรวจจับ (ได้ START ใหม่)
volatile uint32_t      sensorTicks  = 0;       // sensorTask นับทุกรอบ — ลูปใช้เช็คว่า task ยังเดินอยู่
uint32_t      seenPassCount    = 0;            // passCount ที่ลูปแปลงเป็นเวลาจริงแล้ว
uint64_t      lastPassEpochMs  = 0;            // เวลาจริง (epoch ms) ของครั้งล่าสุดที่ส่งขึ้นเว็บ
unsigned long lastPassSeenMs   = 0;            // millis() ของครั้งล่าสุด (ขับไฟขาวค้าง)
unsigned long lastSettingsPoll = 0;

// ---------- ขั้นตอนของหน้ากล้อง (โหมดกล้อง — ขับไฟแทนสถานะเก้าอี้) ----------
// ⚠️ ชื่อ phase ต้องตรงกับ CameraPhase ใน gait-web/src/lib/firebase.ts
enum CamPhase : uint8_t { CAM_UNKNOWN, CAM_OFF, CAM_NO_SIDE, CAM_HIDDEN, CAM_WAITING, CAM_READY, CAM_RUNNING, CAM_COOLDOWN };
CamPhase      camPhase        = CAM_UNKNOWN;
bool          camFresh        = false;   // หน้ากล้องยังส่งสถานะอยู่ (ไม่ได้ปิดไป)
unsigned long camFreshAt      = 0;       // millis() ที่อ่านได้ว่าสดครั้งล่าสุด (กันเน็ตหลุดแล้วค้างค่าเก่า)
unsigned long camRunSinceMs   = 0;       // millis() ที่เห็นว่าเริ่มจับเวลา — คนผ่านหลังจากนี้ = ขาวค้าง
unsigned long camResultUntil  = 0;       // โชว์สีผลถึงเมื่อไร (millis)
RiskLevel     camRisk         = RISK_LOW;
unsigned long lastCamPoll     = 0;
// เปิดหน้าเว็บหลายแท็บ/หลายเครื่อง ทุกหน้าเขียนเอกสารเดียวกันสลับกัน — ถ้าเพิ่งเห็นหน้ากล้องที่ใช้งานจริง
// (รอนั่ง/พร้อม/จับเวลา/พัก) ภายในช่วงนี้ ค่า ถูกซ่อน/ยังไม่ได้ภาพ ที่แทรกมาถือว่าเป็นของหน้าอื่น ไม่ให้ทับ
// (หน้ากล้องตัวจริงส่งทุก 10 วิ ถ้าถูกซ่อนจริง ไฟจะเปลี่ยนหลังช่วงนี้)
#define CAM_ACTIVE_HOLD_MS  15000
unsigned long camActiveAt     = 0;
...
volatile bool      wifiCfgTentative = false;   // ค่าที่ได้เป็นแบบ "ลองดู" (TUGWTRY)
struct_wifi_config wifiCfgInbox;
String             lastAppliedSsid, lastAppliedPass;   // ใช้กันเอาค่าเดิมไปต่อซ้ำ
...
// (ctags ของ Arduino เป็นโปรแกรม Intel — macOS รุ่นที่ไม่มี Rosetta รันไม่ได้)
...
bool        cameraDrivesLight();
LightPhase  riskLight(RiskLevel r);
LightPhase  cameraLightPhase();
void        pollCameraStatus();
unsigned long camPollGap();
...
// เวลาจริงละเอียดระดับ ms (0 = NTP ยังไม่ sync)
uint64_t getEpochMs() {
  struct timeval tv;
...
bool chairPresent() {
  return lastChairAck != 0 && (millis() - lastChairAck < CHAIR_ABSENT_MS);
}
...
// โหมดเฝ้าตลอดไม่ต้องมีเก้าอี้ — ครบแค่ WiFi + คลาวด์ก็พร้อม
bool deviceReady() {
  return wifiConnected && cloudOk && (chairLinkOk() || freeRun);
}
...
// โหมดกล้อง: ใช้ขั้นตอนของหน้ากล้องขับไฟ (สีเดียวกับโหมดฮาร์ดแวร์ทุกขั้น)
bool cameraDrivesLight() {
  return timingCamera && camPhase != CAM_UNKNOWN;
}

LightPhase riskLight(RiskLevel r) {
  if (r == RISK_LOW)      return LIGHT_RESULT_LOW;
  if (r == RISK_MODERATE) return LIGHT_RESULT_MOD;
  return LIGHT_RESULT_HIGH;
}

LightPhase cameraLightPhase() {
  bool fresh = camFresh && millis() - camFreshAt < (unsigned long)CAM_STALE_HIDDEN_SEC * 1000UL;
  if (fresh && camResultUntil && (long)(camResultUntil - millis()) > 0) return riskLight(camRisk);
  if (!fresh || !wifiConnected || !cloudOk) return LIGHT_NOT_READY;   // หน้ากล้องปิด/เน็ตหลุด = จับเวลาไม่ได้
  switch (camPhase) {
    case CAM_RUNNING: {
      // ผ่านจุดหมุนตัว: บอร์ดนี้รู้เองทันที (ไม่ต้องรอหน้ากล้อง) หรือเก้าอี้บอก (ถ้าเปิดไว้)
      // เผื่อ 2 วิ: คนผ่านก่อนบอร์ดรู้ว่าเริ่มจับเวลา (อ่านสถานะหน้ากล้องช้ากว่าจริงได้ราว 1-2 วิ)
      bool passed = awaitingFinish || (lastPassSeenMs && (long)(lastPassSeenMs - camRunSinceMs) >= -2000);
      return passed ? LIGHT_PASSED : LIGHT_WALKING;
    }
    case CAM_READY:    return LIGHT_STAND_UP;
    case CAM_WAITING:  return LIGHT_READY;
    case CAM_COOLDOWN: return LIGHT_READY;       // สีผลโชว์ 10 วิแรกไปแล้วด้านบน
    default:           return LIGHT_NOT_READY;   // ยังไม่ได้ภาพกล้องข้าง / หน้ากล้องถูกซ่อน / ปิด
  }
}

LightPhase currentLightPhase() {
  if (cameraDrivesLight()) return cameraLightPhase();
  ...
  // เก้าอี้หายไปแล้ว — สถานะที่ได้ยินครั้งสุดท้ายเป็นของเก่า ห้ามเอามาขับไฟค้างไว้
  uint8_t cs = chairPresent() ? chairState : CHAIR_UNKNOWN;
  if (awaitingFinish || cs == CHAIR_RETURNING)                      return LIGHT_PASSED;
  if (currentState == CP_DETECTING || cs == CHAIR_RUNNING)          return LIGHT_WALKING;
  // เฝ้าตลอด: ไม่มีใครบอกขั้นตอน — ขาวค้างสั้น ๆ ให้เห็นว่าบอร์ดจับได้ว่ามีคนเดินผ่าน
  if (freeRun && lastPassSeenMs && millis() - lastPassSeenMs < PASS_LIGHT_MS) return LIGHT_PASSED;
  if (!deviceReady())                                              return LIGHT_NOT_READY;
  if (cs == CHAIR_READY)                                           return LIGHT_STAND_UP;
  return LIGHT_READY;
}
...
bool sendHeartbeat() {
  ...
  // โหมดกล้อง: เวลาจริงของครั้งล่าสุดที่เห็นคนเดินผ่าน — เว็บเอาไปลบกับเวลาเริ่มของกล้อง = ขาไป
  // เว็บแยกครั้งใหม่ด้วยค่า pass_at_ms ที่เปลี่ยน (pass_count เริ่ม 0 ใหม่ทุกครั้งที่บูต)
  content.set("fields/free_run/booleanValue",     (bool)freeRun);
  content.set("fields/timing_mode/stringValue",   timingCamera ? "camera" : "hardware");
  content.set("fields/pass_count/integerValue",   String((unsigned long)seenPassCount));
  char passBuf[24];
  snprintf(passBuf, sizeof(passBuf), "%llu", (unsigned long long)lastPassEpochMs);
  content.set("fields/pass_at_ms/integerValue",   passBuf);
  ...
  if (!Firebase.Firestore.patchDocument(
        ...
        "free_run,timing_mode,pass_count,pass_at_ms")) {
...
// ขั้นตอนของหน้ากล้อง (device_status/camera) — ใช้ขับไฟในโหมดกล้อง
unsigned long camPollGap() {
  switch (camPhase) {
    case CAM_READY:
    case CAM_RUNNING:  return CAM_POLL_ACTIVE_MS;
    case CAM_WAITING:
    case CAM_COOLDOWN: return camFresh ? CAM_POLL_NEAR_MS : CAM_POLL_IDLE_MS;
    default:           return CAM_POLL_IDLE_MS;
  }
}
...
void pollCameraStatus() {
  ...
  if (!Firebase.Firestore.getDocument(&fbdo, FIREBASE_PROJECT_ID, "(default)",
                                      "device_status/camera", "phase,last_seen,last_duration_ms")) {
    return;
  }
  FirebaseJson payload;
  payload.setJsonData(fbdo.payload());
  FirebaseJsonData r;
  payload.get(r, "fields/phase/stringValue");
  String ph = r.success ? r.to<String>() : String("off");
  long lastSeen = readIntField(payload, "fields/last_seen/integerValue");
  long lastDur  = readIntField(payload, "fields/last_duration_ms/integerValue");

  CamPhase next = ph == "running"  ? CAM_RUNNING
                : ph == "cooldown" ? CAM_COOLDOWN
                : ph == "ready"    ? CAM_READY
                : ph == "waiting"  ? CAM_WAITING
                : ph == "no_side"  ? CAM_NO_SIDE
                : ph == "hidden"   ? CAM_HIDDEN
                                   : CAM_OFF;
  // ความสดเทียบ last_seen (นาฬิกาเครื่องที่เปิดหน้ากล้อง) กับ NTP ของบอร์ด — เผื่อนาฬิกาต่างกันไว้ 30 วิ
  long age   = (long)getEpoch() - lastSeen;
  long limit = next == CAM_HIDDEN ? CAM_STALE_HIDDEN_SEC : CAM_STALE_SEC;
  bool fresh = lastSeen > 0 && getEpoch() != 0 && age > -30 && age < limit && next != CAM_OFF;

  bool active = fresh && next >= CAM_WAITING;   // waiting/ready/running/cooldown = หน้ากล้องที่ใช้งานอยู่
  if (active) {
    camActiveAt = millis();
  } else if (camActiveAt && millis() - camActiveAt < CAM_ACTIVE_HOLD_MS) {
    return;   // หน้าอื่นที่ไม่ได้ใช้กล้องเขียนทับ — คงขั้นตอนของหน้ากล้องตัวจริงไว้
  }

  if (next == CAM_RUNNING && camPhase != CAM_RUNNING) camRunSinceMs = millis();
  // จบรอบ: กำลังจับเวลา → พัก และมีเวลารอบนี้ (0 = รอบที่ไม่บันทึก ไม่โชว์สีผล)
  if (camPhase == CAM_RUNNING && next == CAM_COOLDOWN && lastDur > 0) {
    camRisk        = riskLevelOf(lastDur / 1000.0f);
    camResultUntil = millis() + RESULT_DISPLAY_MS;
    Serial.printf("  [Camera] จบรอบ %.2f วิ → ไฟผล %s\n", lastDur / 1000.0f, riskName(camRisk));
  }
  if (next != CAM_COOLDOWN) camResultUntil = 0;
  if (next != camPhase) {
    Serial.printf("  [Camera] หน้ากล้อง: %s%s\n", ph.c_str(), fresh ? "" : " (ไม่ได้ส่งสถานะแล้ว)");
    heartbeatDue = true;   // สีไฟเปลี่ยน — ให้เว็บเห็นสีเดียวกับหลอดจริง
  }
  camPhase = next;
  camFresh = fresh;
  if (fresh) camFreshAt = millis();
}

// แหล่งจับเวลาที่เลือกบนเว็บ (device_commands/settings.timing_source) — กล้อง = เฝ้าตลอด
void pollTimingSource() {
  if (WiFi.status() != WL_CONNECTED || !Firebase.ready()) return;
  if (!Firebase.Firestore.getDocument(&fbdo, FIREBASE_PROJECT_ID, "(default)",
                                      "device_commands/settings", "timing_source")) {
    return;   // ยังไม่เคยตั้ง — คงค่าเดิม (ค่าเริ่มต้นของเว็บคือกล้อง แต่ไม่เดาเอง รอเว็บตั้ง)
  }
  FirebaseJson payload;
  payload.setJsonData(fbdo.payload());
  FirebaseJsonData r;
  payload.get(r, "fields/timing_source/stringValue");
  if (!r.success) return;
  bool cam = (r.to<String>() == "camera");
  if (cam != timingCamera) {
    timingCamera = cam;
    heartbeatDue = true;
    if (!cam) { camPhase = CAM_UNKNOWN; camFresh = false; camResultUntil = 0; }
    Serial.printf("  [Mode] แหล่งจับเวลาบนเว็บ: %s\n", cam ? "กล้อง → เฝ้าตลอด" : "ฮาร์ดแวร์ → รอ START จากเก้าอี้");
  }
}
...
void OnDataRecv(ESPNOW_RECV_CB_ARG, const uint8_t *incomingData, int len) {
  ...
  if (len == sizeof(struct_wifi_config)) {
    ...
    bool tentative = (strcmp(cfg.magic, TUG_WIFI_MAGIC_TRY) == 0);
    if (!tentative && strcmp(cfg.magic, TUG_WIFI_MAGIC) != 0) return;

    lastChairAck     = millis();
    wifiCfgInbox     = cfg;
    wifiCfgTentative = tentative;
  ...
  if (strcmp(in.command, "START") == 0) {
    ...
    hwPassFlag      = false;
    detectReset     = true;       // ล้างช่วงพักของโหมดเฝ้าตลอด — START คือรอบจริงเสมอ
...
void applyPendingWifi() {
  ...
  // กำลังเฝ้าคนเดินผ่าน — เก็บค่าไว้ก่อน สลับ WiFi ตอนนี้ ESP-NOW จะหลุดกลางการทดสอบ
  if (currentState == CP_DETECTING) return;
  wifiCfgPending = false;

  String ssid(wifiCfgInbox.ssid);
  String pass(wifiCfgInbox.pass);
  bool   tentative = wifiCfgTentative;

  // ต่อวงนี้อยู่แล้ว (เช่น เก้าอี้ยืนยันค่าหลังต่อติด หรือเก้าอี้เพิ่งรีบูต) — ไม่ต้องต่อใหม่
  if (portal.isConnected() && WiFi.SSID() == ssid && portal.currentPass() == pass) {
    Serial.print("  [Provision] เก้าอี้ส่งค่า WiFi มา แต่ใช้วงนี้อยู่แล้ว : ");
    Serial.println(ssid);
    if (!tentative) portal.rememberNetwork(ssid, pass);   // ไม่เขียน flash ถ้าอยู่หัวรายการแล้ว
    return;
  }
  // กำลังต่อค่าเดียวกันนี้อยู่ (เก้าอี้ส่งซ้ำ หรือยืนยันหลังตัวเองต่อติด) — ปล่อยให้ทำจนจบ
  // ถ้าเป็นค่ายืนยัน ให้จำไว้เลย: ถ้ารอบนี้ต่อไม่ติด portal จะวนกลับมาลองวงนี้ก่อน
  if (portal.isConnecting() && ssid == lastAppliedSsid && pass == lastAppliedPass) {
    if (!tentative) portal.rememberNetwork(ssid, pass);
    return;
  }

  lastApplyTime   = millis();
  lastAppliedSsid = ssid;
  lastAppliedPass = pass;

  Serial.println();
  Serial.println("========================================");
  Serial.print  (tentative ? "  [Provision] เก้าอี้เปลี่ยน WiFi — ลองต่อตาม : "
                           : "  [Provision] ได้ค่า WiFi จากบอร์ดเก้าอี้ : ");
  Serial.println(ssid);                   // ไม่พิมพ์รหัสผ่านลง Serial
  Serial.println("========================================");

  portal.applyCredentials(ssid, pass, !tentative);
}
...
// ==========================================================
// sensorTask — อ่านเซนเซอร์ + ตรวจคนเดินผ่าน แยกจากลูปหลัก
// ==========================================================
// priority สูงกว่าลูป (1) บน core เดียวกัน: ตอนลูปติดคุย Firestore (HTTPS บล็อกได้เป็นวินาที)
// task นี้ยังแทรกมาอ่านเซนเซอร์ได้ตามรอบ ไม่พลาดจังหวะคนเดินผ่าน
// เวลาที่จดคือ millis() ของ "เฟรมแรกที่เห็นคนในระยะ" (ไม่ใช่ตอนยืนยันครบ 300 ms)
void sensorTask(void*) {
  bool          debouncing = false;
  unsigned long debStart   = 0;
  bool          needClear  = false;   // ช่วงพักหลังนับไปแล้ว (รอคนพ้นระยะ)
  unsigned long clearStart = 0;
  unsigned long lastHit    = 0;

  for (;;) {
    float d = getDistance();          // ยิงจริงทุก PING_GAP_MS ระหว่างนั้นคืนค่าเดิม
    unsigned long now = millis();
    sensorTicks = sensorTicks + 1;
    // ขับจังหวะกระพริบจาก task นี้ด้วย — ลูปอาจติดคุย Firestore ครึ่งวินาที ไฟจะกระพริบสะดุด
    // (ลูปยังเป็นคนเลือกรูปแบบไฟใน updateStatusLight เหมือนเดิม)
    driveLed();
    ...
    bool hwArmed = (currentState == CP_DETECTING);
    bool armed   = hwArmed || freeRun;
    bool inRange = d > 0.0 && d < distDetectCm;

    if (needClear) {
      if (inRange) clearStart = now;
      else if (now - clearStart >= FREE_RUN_CLEAR_MS && now - lastHit >= FREE_RUN_REFRACTORY_MS) needClear = false;
    }

    if (!armed || needClear || hwPassFlag) {
      debouncing = false;
    } else if (inRange) {
      if (!debouncing) {
        debouncing = true;
        debStart   = now;
      } else if (now - debStart >= DEBOUNCE_DETECT_MS) {
        passMillis = debStart;
        passCount  = passCount + 1;
        if (hwArmed) hwPassFlag = true;
        debouncing = false;
        needClear  = true;
        clearStart = now;
        lastHit    = now;
...
void setup() {
  ...
  // เซนเซอร์เดินเองใน task แยก — ต้องเริ่มหลัง ESP-NOW พร้อม (task อ่าน currentState ที่ callback เขียน)
  if (xTaskCreatePinnedToCore(sensorTask, "cp_sensor", 4096, nullptr, 2, nullptr, 1) != pdPASS) {
    Serial.println("[ERROR] สร้าง sensorTask ไม่สำเร็จ — รีบูต");
    delay(1000);
    ESP.restart();
  }
...
// ==========================================================
void loop() {
  float distance = filteredDistance;   // sensorTask เป็นคนอ่านเซนเซอร์ ลูปแค่เอาค่าล่าสุดไปแสดง/รายงาน
  unsigned long now = millis();

  // --- โหมดเฝ้าตลอด: เว็บเลือกจับเวลาจากกล้อง หรือไม่มีเก้าอี้ให้รอ START ---
  bool fr = timingCamera || !chairPresent();
  if (fr != freeRun) {
    freeRun      = fr;
    heartbeatDue = true;
    Serial.printf("  [Mode] %s\n", fr ? "เฝ้าตลอด (จับเวลาจากกล้อง / ไม่มีเก้าอี้)" : "รอ START จากเก้าอี้ (ฮาร์ดแวร์)");
  }

  // --- เห็นคนเดินผ่าน (จาก sensorTask) → แปลงเป็นเวลาจริงแล้วส่งขึ้นเว็บ ---
  // แปลงตอนนี้จาก "ตอนนี้ลบด้วยเวลาที่ผ่านไป" จึงถูกต้องแม้ลูปเพิ่งว่างจากการคุย Firestore
  // หรือ NTP เพิ่ง sync หลังคนเดินผ่าน
  uint32_t pc = passCount;
  if (pc != seenPassCount) lastPassSeenMs = passMillis;   // ไฟขาวค้างได้ทันที แม้ NTP ยังไม่ sync
  if (pc != seenPassCount) {
    uint64_t nowMs = getEpochMs();
    if (nowMs != 0) {
      unsigned long pm = passMillis;
      seenPassCount   = pc;
      lastPassSeenMs  = pm;
      lastPassEpochMs = nowMs - (uint64_t)(millis() - pm);
      heartbeatDue    = true;
      Serial.printf("  [PASS] เห็นคนเดินผ่าน #%lu (%s)\n", (unsigned long)pc,
                    currentState == CP_DETECTING ? "รอบของเก้าอี้" : "เฝ้าตลอด");
    }
  }
  ...
  if (now - lastPrintTime >= SERIAL_INTERVAL_MS) {
    ...
    if (sensorTicks == prevTicks)           Serial.print("  [Sensor: หยุดทำงาน!]");
    prevTicks = sensorTicks;
    if (freeRun)                            Serial.print("  [เฝ้าตลอด]");
  ...
  if (currentState != CP_DETECTING) {
    ...
    if (now - lastSettingsPoll >= SETTINGS_POLL_INTERVAL_MS) {
      pollTimingSource();
      lastSettingsPoll = now;
    }
    if (timingCamera && now - lastCamPoll >= camPollGap()) {
      pollCameraStatus();
      lastCamPoll = now;
    }
  }
  ...
  switch (currentState) {
    ...
    case CP_DETECTING:
      ...
      // sensorTask ยืนยันแล้วว่าผู้ทดสอบมาถึงจุดหมุนตัว (ลูปไม่คุย Firestore ช่วงนี้ จึงส่งได้ทันที)
      if (hwPassFlag) {
        sendCommand("CHECKPOINT", 0.0);   // แนบ runId ของรอบนี้ไปด้วยอัตโนมัติ
        currentState     = CP_IDLE;
        hwPassFlag       = false;         // ล้างหลังเปลี่ยนสถานะ — sensorTask จะไม่นับซ้ำในรอบนี้
        debounceActive   = false;
