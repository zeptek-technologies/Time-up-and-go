...
public:
  ...
  // ฝั่งที่ "รับ" เรียกตัวนี้ — task จะลองต่อทันที
  // เรียกจาก loop() เท่านั้น ห้ามเรียกจาก callback ของ ESP-NOW
  //
  // remember=true  → จำลง NVS ก่อนลองต่อ (ค่าที่ยืนยันแล้วว่าใช้ได้)
  // remember=false → ค่า "ลองดู" จำเฉพาะเมื่อต่อสำเร็จ ถ้าไม่ติดจะกลับไปต่อวงที่จำไว้ทันที
  //                  (ใช้ตอนเก้าอี้ส่งค่าที่ผู้ใช้เพิ่งกรอก ซึ่งยังไม่รู้ว่ารหัสถูกไหม)
  void applyCredentials(const String& ssid, const String& pass, bool remember = true) {
    if (ssid.length() == 0) return;
    _Lock lock(_mtx);
    _reqSsid     = ssid;
    _reqPass     = pass;
    _reqRemember = remember;
    _reqApply    = true;
  }

  // เรียกทุกครั้งที่ผู้ใช้กด "เชื่อมต่อ" จากหน้าเว็บ — ก่อนที่วิทยุจะออกจากช่องเดิม
  // บอร์ดเก้าอี้ใช้ส่งค่าใหม่ให้ Checkpoint ในจังหวะที่สองบอร์ดยังอยู่ช่องเดียวกัน
  // ⚠️ ถูกเรียกจาก task ของ portal (ไม่ใช่ลูปหลัก) — ห้ามบล็อกนาน
  using ConnectHook = std::function<void(const String& ssid, const String& pass)>;
  void onUserConnect(ConnectHook hook) { _onUserConnect = hook; }
...
private:
  ...
  bool          _reqRemember = true;
  String        _reqSsid, _reqPass;
  ConnectHook   _onUserConnect;
  ...
  bool          _tentative      = false;   // กำลังลองค่าที่ยังไม่ได้จำ (applyCredentials remember=false)
  ...
  void _processRequests() {
    ...
    if (_reqApply) {
      ...
      bool remember;
      {
        ...
        s = _reqSsid;
        p = _reqPass;
        remember = _reqRemember;
        _reqSsid = _reqPass = "";
        _reqRemember = true;
        _connecting = true;                  // ให้ isConnecting() เป็น true ต่อเนื่อง ไม่มีช่องว่าง
        _reqApply   = false;
      }
      if (remember) rememberNetwork(s, p);
      _retryGap  = TUG_PORTAL_RETRY_MS;
      // จำไว้แล้ว → อยู่หัวรายการ ถ้าต่อไม่ติดให้ไล่วงถัดไป (index 1 เป็นต้นไป)
      // ยังไม่จำ → ถ้าต่อไม่ติดให้กลับไปเริ่มที่หัวรายการ (ดู _tickConnect)
      _autoIndex = remember ? 0 : -1;
  ...
  void _tickConnect() {
    if (_connecting) {
      ...
      // ค่า "ลองดู" ใช้ไม่ได้ — กลับไปต่อวงที่จำไว้ทันที ไม่ปล่อยให้ออฟไลน์รอครบรอบ
      if (_tentative) {
        _tentative = false;
        if (_savedCount > 0) {
          Serial.println("  [Portal] ค่าใหม่ใช้ไม่ได้ — กลับไปต่อเครือข่ายที่จำไว้");
          _beginAttempt(0);
          return;
        }
      }
  ...
  void _routes() {
    ...
    _server.on("/api/connect", HTTP_POST, [this]() {
      ...
      _retryGap  = TUG_PORTAL_RETRY_MS;
      _tentative = false;
      if (_onUserConnect) _onUserConnect(ssid, pw);   // ต้องก่อน _startConnect — วิทยุยังอยู่ช่องเดิม
