...
## Patient registry and avatars
PatientAvatar.tsx is the shared identity mark for dashboard, registry rows and the selected profile. It always shows Icons.tsx's user outline inside a circle (40px list / 64px detail), never initials. Recorded gender หญิง uses pink #fce7f3 with #be185d; ชาย uses existing primary-100/700 blue; other/unspecified uses neutral #f1f5f9/#64748b. Values are owned once in components/patient-avatar.css. No gender inference from names. Icons are decorative because the adjacent identity text supplies the name and profile gender.
The registry uses a separate 300px roster and fluid detail panel with a 24px gap inside a 1280px content width. The roster no longer reserves an empty 540px column. Profile padding is 20px on every edge; actions wrap below the identity when space is limited. Below 760px panels stack and names wrap. Roster and history overflow surfaces are keyboard focusable and height-bounded; form dimensions remain unchanged.

## Device settings and cooldown countdown (2026-09-11)
DeviceSettingsSection.tsx is the last dashboard section (key devices, label ตั้งค่าอุปกรณ์), one card per board in a two-column grid that stacks below 960px. Each distance uses a native range slider paired with a −/number/+ stepper (42px controls), a 0–150 cm zone bar (primary-500 = sit, striped neutral = hysteresis gap, teal-500 = stand/arrived, neutral = far) with a marker for the board's latest reading, and a single primary "บันทึกค่า" action. The sync line reports saved vs board-applied values in text, never by color alone. Styles live in components/device-settings.css and reuse existing tokens. The live screen shows the cooldown countdown as a small right-aligned line under the progress rail, outside the atomic live region.
