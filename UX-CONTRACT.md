...
## Camera-based TUG timing contract (2026-09-23)
- Canonical owners: lib/postureDetector.ts (sitting/transition/standing from hip angle, knee angle and hip lift; 2-of-3 vote), lib/cameraTugTimer.ts (waiting → ready → running → cooldown), lib/timingMerge.ts (one row per trial), CameraPage.tsx (wiring, recording ownership, status publishing), hooks/useCameraStatus.ts (live screen). Tests: gait-web/tests/cameraTimer.mjs (synthetic postures only).
- The side camera is the primary timer; the chair sensor keeps timing in parallel as backup and cross-check. Timer clock = frame capture time (performance.now base), never media time, so stream restarts cannot break durations.
- Start/end are backdated from a 3 s posture history to the frame the hip left / returned to the seat. Sitting back within 2.5 s without standing = fidget (cancel); runs under 4 s are rejected; runs over 120 s are aborted. Person may leave the frame while walking.
- Camera timings are stored as tug_results/cam_* (device "camera") — no new collection or rules. subscribeResults pairs them with the chair row by session + trial + finished within 90 s; paired rows keep the chair document id and show camera time with the chair time for comparison. Unpaired camera rows appear on their own. Order of arrival does not matter.
- Recording ownership (camera / chair / manual): only the owner that started a recording may finish it, preventing duplicate gait uploads.
- device_status/camera carries phase + elapsed_ms (1 s while running, 10 s idle). The live screen measures freshness and elapsed time with its own clock on receipt, not the writer's clock.
- Posture thresholds are initial values; tune them against recorded side-view clips of real participants before clinical use.
- Timing source (camera / hardware) is one shared setting at device_commands/settings.timing_source (default camera), chosen in "ตั้งค่าอุปกรณ์". It is locked while a run is in progress and applies from the next run; earlier results are unchanged. Hardware mode: the camera tab publishes phase "off", does not time or save cam_* docs, and only records gait; the live screen uses the chair/checkpoint only. Camera mode with no camera tab open falls back to the chair and says so in the live-screen footer.
- The camera cooldown matches the chair cooldown (cooldown_left_ms is published). Rejected/aborted camera runs clear last_duration_ms, so the live screen shows "ไม่บันทึกรอบนี้" instead of the previous time.
- In camera mode, whenever the camera tab is open it publishes a phase: no_side (no side-camera frames), hidden (tab hidden, so the browser pauses video processing) or the timer phase. The live screen then follows the camera only, shows a camera chip, and never falls back to "device offline". Hidden has a 90 s staleness window because background tabs are throttled.
- Checkpoint firmware v4 (checkpoint-3.x, ESP_Checkpoint_v4, based on v3) supports three setups: camera only; camera + checkpoint (chair optional); chair + checkpoint hardware (unchanged). In camera mode (read from device_commands/settings), or when no chair has been heard for 15 s, the checkpoint watches continuously and publishes each pass as device_status/checkpoint.pass_at_ms (NTP epoch ms, taken at the first in-range sample). A sensor FreeRTOS task keeps detection independent of Firestore calls. Its recovery AP (after 90 s without WiFi) now serves the setup page, so it can be configured without a chair.
- The camera page maps its capture-clock start to server time using serverTimestamp round trips (ClockOffset, min-RTT sample), then picks the first pass that is ≥1 s after start and ≥1 s before the end as the outbound split. The split is saved into the cam_* doc, waiting up to 4 s for a late pass while the checkpoint is online. Merge prefers the chair's split when the chair measured one. The live screen shows "passed turning point" when a pass arrives (local receipt time) during a camera run.
- Checkpoint LED in camera mode follows the camera page (device_status/camera, polled every 1 s during ready/running, 2 s during waiting/cooldown, 8 s otherwise), using the same colors as hardware mode:
  - waiting: steady blue
  - ready: blinking green
  - running: blinking white; steady white once the board itself sees the pass
  - result: green/yellow/red for 10 s after running → cooldown with a duration
  - camera closed / hidden / no side camera: blinking blue
  - A hidden/no_side reading within 15 s of an active one is ignored, because other tabs write the same doc.
  - LED blinking runs in the sensor task, so Firestore calls don't stall it.
- Camera status is published only by pages that have opened a camera. The live screen keeps the latest status per client_id (client_ms dedupes legacy writers) and shows the most active fresh one.
